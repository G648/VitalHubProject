...
export const ClockCard = styled.View`
  ...
  background-color: ${(props) =>
    props.situation == "Pendentes" ? scheduledBgColor : defaultBgColor};
`;
...
export const ButtonSelectedCard = styled.TouchableOpacity`
  ...
  border-color: ${({ isSelected = false }) =>
    isSelected ? APP_COLORS.secondary : "transparent"};
  width: 100%;
  border-radius: 8px;
  margin-bottom: 5%;
`;
...
export function CardUser({
  ...
  return (
    ...
    >
      <CardsUser marginBottom={marginBottomCard}>
        <ProfileImage
          width={widthImage}
          height={heightImage}
        ...
        <ContainerFlex>
          <ProfileName marginBottomName={marginBottomName}>
            {nameUser}
          </ProfileName>
          ...
          <ViewRow>
            ...
            <ClinicAvaliation>{clinicAvaliation}</ClinicAvaliation>
            <ButtonCard activeOpacity={0.6} onPress={onPress}>
              {situation === "Pendentes" ? (
