...
const PatientHome = ({ navigation, role, jti }) => {
  ...
  async function ListarConsultas() {
    ...
    await api
      .get(
        `/api/${url}/BuscarPorData?data=${dataConsulta}&id=${token.jti}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleCardPress = (selectedSituation, userData) => {
    selectedSituation == "Agendadas"
      ...
      : navigation.navigate("MedicalRecordPage", { userData: userData });
    setSelectedUserData(userData);
  };

  const handleCardPressInfoDoctor = (selectedSituation, userData) => {
    selectedSituation == "Realizadas"
  ...
  async function handleCallNotifications() {
    const { status: existingStatus } =
      await Notifications.getPermissionsAsync(); //obter o status da permissão
        ...
        renderItem={({ item }) => {
          <CardUser
            ...
            onPressBorder={() =>
              ...
              item.situation === "Agendadas"
                ? handleCardPressInfoDoctor(selectedButton, item)
                : null
            }
          />;
        }}
