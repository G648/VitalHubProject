...
namespace WebAPI.Controllers
{
    ...
    public class ExameController : ControllerBase
    {
        ...
        public ExameController(IExameRepository exameRepository, OcrSettings ocrSettings)
        {
        ...
        public async Task<IActionResult> Post([FromForm] ExameViewModel exameViewModel)
        {
            try
            {
                ...
                using (var stream = exameViewModel.Imagem.OpenReadStream())
                {
                    ...
                    var result = await _ocrSettings.RecognizeTextAsync(stream);
            ...
            catch (Exception ex)
            {
                ...
                return StatusCode(500, $"Erro ao processar a imagem: {ex.InnerException}");
            }
        }
